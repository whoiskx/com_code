...
class Mobile(object):
    ...
    def run(self):
        ...
        #     f.write(resp.text)
        import re
        results = re.search('var msgList =.*?\';', resp.text).group()
        from xml.sax.saxutils import unescape
        #
        # import html.parser
        # n = html.parser.HTMLParser().unescape(results[0])
        import html
        new1 = html.unescape(results[0])

        # from html.parser import HTMLParser
        # new3 = HTMLParser().unescape(results[0])

        # new = unescape(results[0])

        # title_urls = re.findall(r'http:\\/\\/mp.weixin.qq.com\\/s.*?#wechat_redirect', results[0])
        # print(title_urls[0])

        title_urls = re.findall(r'mp.weixin.qq.com.*?#wechat_redirect', new1)
        prefix = 'https://mp.weixin.qq.com/s?'
        for url_title in title_urls:
            url_title = prefix + url_title.replace('amp;', '').replace(r'mp.weixin.qq.com\\/s?', '')
            print(url_title)
        results.split(";")
