...
class Article(object):
    ...
    def create(self, url, account_model, proxies):
        ...
        self.title = e('.rich_media_title').text().replace(' ', '').replace('&nbsp;', '').replace('&quot;', '')
        # todo 分享的和视频
...
class Account(object):
    ...
    def get_account_id(self):
        get_account_id = 'http://60.190.238.178:38010/search/common/wxaccount/select?' \
            ...
            self.account)
        url_resp = requests.get(get_account_id, timeout=21)
    ...
    def uploads(backpack_list):
        ...
        if backpack_list:
            ...
            while True:
                ...
                try:
                    log('start uploads')
                    r = requests.post(sever1, data=body, timeout=21)
                    if r.status_code == 200:
                        log('uploads server1 successful')
                except Exception as e:
                    log('uploads http error1 {}'.format(e))
                try:
                    r2 = requests.post(sever2, data=body, timeout=21)
        ...
        if backpack_list:
            ...
            while True:
                ...
                try:
                    ...
                    r = requests.post(server, data=json.dumps(datacenter_relay), timeout=21)
                    if r.status_code == 200:
                        log('uploads datacenter_relay server successful')
                        break
                except Exception as e:
                    log('uploads datacenter_relay server error: {}'.format(e))
                try:
                    log('start uploads datacenter_relay backup')
                    r = requests.post(server_backup, data=json.dumps(datacenter_relay), timeout=21)
                    if r.status_code == 200:
                        log('uploads datacenter_relay server backup successful')
                        break
                    log('数据中心发包异常')
                except Exception as e:
                    log('uploads datacenter_relay server backup error: {}'.format(e))
                count += 1
        ...
        if backpack_list:
            ...
            while True:
                ...
                try:
                    ...
                    r = requests.post(sever, data=json.dumps(datacenter_unity), timeout=21)
                    if r.status_code == 200:
