...
headers = {
    'Host': 'graph.facebook.com', 'Connection': 'keep-alive', 'Cache-Control': 'max-age=0',
...
def _main():
    ...
    with open("fbtoken", "r") as f:
        for i in range(290):
            ...
            resp = requests.get(url_add, headers=headers)
            print(resp.status_code)
            print(resp.text)
            print("=======================")
            if resp.status_code != 403:
                print(token, resp.status_code, "sucessful============!!!!!!!!!!!!!!")
            f_write.write("{}:\n {}".format(token, resp.text))
    f_write.close()
