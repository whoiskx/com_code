...
class MysqlMixin(object):
    ...
    mysql_server = None
    mysql_batch_size = None
    mysql_cursor = None

    def start_requests(self):
        """returns start requests from mysql batabase."""
        return self.read_mysql()

    def setup_mysql(self, crawler=None):
        """Setup mysql connection

        This should be called after the spider has set its crawler object.
        """
        if self.mysql_server is not None:
            return
        ...
        # TODO: still not support reading batch size
        if self.mysql_batch_size is None:
            self.mysql_batch_size = settings.getint(
                'MYSQL_BATCH_SIZE',
                defaults.MYSQL_BATCH_SIZE,
            )

        try:
            self.mysql_batch_size = int(self.mysql_batch_size)
...
class MysqlRedisSpider(MysqlMixin, RedisMixin, Spider):
    ...
    def from_crawler(self, crawler, *args, **kwargs):
        obj = super(MysqlRedisSpider, self).from_crawler(crawler, *args, **kwargs)
        obj.setup_mysql(crawler)
