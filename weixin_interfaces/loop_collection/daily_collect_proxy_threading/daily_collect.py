...
class AccountHttp(object):
    ...
    def account_homepage(self):
        # 搜狗搜索页面（验证码） -> 微信历史页（验证码）-> 文章详情页（二维码）
        count = 0
        while True:
            count += 1
            if count > 25:
                log.info('多次账号异常，跳过账号:{}'.format(self.search_name))
                return
            ...
            if '搜狗' in e('title').text():
                ...
                #     finally:
                #         lock.release()
                if crack_result and '正确' in crack_result:
    ...
    def crack_sougou(self, url):
        ...
        if re.search('weixin\.sogou\.com', url):
            ...
            if self.proxies:
                r = self.s.post(image_url, cookies=self.cookies, data=data, proxies=self.proxies)
                ret = r.json().get('ret')
                log.info(ret)
                if ret == 0:
